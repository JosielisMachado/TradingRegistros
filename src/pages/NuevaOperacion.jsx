...
const EMOCIONES = [
  ...
  'Eufórico/a','Miedoso/a','Impaciente','Disciplinado/a'
]
...
export default function NuevaOperacion() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold text-green-400 mb-6">➕ Nueva Operación</h1>

      <form onSubmit={handleSubmit} className="space-y-6">

        {/* BLOQUE 1 — Datos principales */}
        <section className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Datos de la operación</h2>
            ...
            <div>
              ...
              {form.activo === 'OTRO' && (
                <input type="text" placeholder="Ej: TSLA, AMZN..."
                  value={form.activoCustom}
                  onChange={e => set('activoCustom', e.target.value)}
        ...
        {/* BLOQUE 2 — Precios */}
        <section className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Precios y gestión de riesgo</h2>
          <div className="grid grid-cols-2 gap-4">
            {[
        ...
        {/* BLOQUE 3 — Resultado */}
        <section className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Tamaño y resultado</h2>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1">Tamaño posición *</label>
              <input type="number" step="any" required
                value={form.tamano_posicion}
        ...
        {/* BLOQUE 4 — Análisis personal */}
        <section className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Análisis personal</h2>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Motivo técnico de entrada</label>
            <textarea rows={3}
              placeholder="Ej: Ruptura de resistencia con volumen, patrón bandera en H1..."
              value={form.motivo_tecnico}
              onChange={e => set('motivo_tecnico', e.target.value)}
          ...
          <div>
            ...
            <textarea rows={2}
              placeholder="Ej: Entré antes de la confirmación, moví el stop..."
              value={form.errores}
              onChange={e => set('errores', e.target.value)}
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-green-500 resize-none"
            />
          </div>
        </section>
        ...
        {/* BLOQUE 5 — Capturas */}
        <section className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Capturas del gráfico</h2>
          <div className="grid grid-cols-2 gap-4">
            {[
              { label: '📸 Antes de la operación', setter: setImagenAntes, val: imagenAntes },
              { label: '📸 Después de la operación', setter: setImagenDespues, val: imagenDespues },
            ].map(({ label, setter, val }) => (
              <div key={label}>
                <label className="block text-sm text-gray-400 mb-2">{label}</label>
                <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-700 rounded-lg cursor-pointer hover:border-green-500 transition-colors">
                  {val
                    ? <span className="text-green-400 text-sm">✅ {val.name}</span>
