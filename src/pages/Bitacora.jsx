...
export default function Bitacora() {
  ...
  const cargarDatos = async () => {
    ...
    let q = supabase
      .from('operaciones')
      .select('*')
      .order(ordenCol, { ascending: ordenDir === 'asc' })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-green-400">Bitacora</h1>
        <Link to="/nueva"
          className="px-4 py-2 bg-green-500 text-gray-950 rounded-lg font-semibold text-sm hover:bg-green-400 transition-colors">
          + Nueva operacion
        </Link>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Total operaciones</p>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-blue-400">{ganadoras} / {ops.length}</p>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex flex-wrap gap-3 items-end">
        ...
        <button onClick={() => { setFiltroActivo(''); setFiltroDir(''); setFiltroEstado('') }}
          className="px-3 py-1.5 text-sm text-gray-400 hover:text-white border border-gray-700 rounded-lg hover:bg-gray-800 transition-colors">
          Limpiar filtros
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-500">
          No hay operaciones.{' '}
          <Link to="/nueva" className="text-green-400 hover:underline">Agrega la primera</Link>
        </div>
      ) : (
                  ...
                  <>
                    <tr key={op.id}
                      ...
                      <td className="px-4 py-3 text-gray-300 whitespace-nowrap">
                        {new Date(op.fecha_hora).toLocaleDateString('es-UY')}
                        <br />
                        <span className="text-xs text-gray-500">
                          {new Date(op.fecha_hora).toLocaleTimeString('es-UY', { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => navigate(`/editar/${op.id}`)}
                            title="Editar"
                            className="text-gray-500 hover:text-yellow-400 transition-colors text-xs px-2 py-1 rounded hover:bg-yellow-900/20">
                            ✎
                          </button>
                          <button
                            onClick={() => eliminar(op.id)}
                            disabled={eliminando === op.id}
                            title="Eliminar"
                      ...
                      <tr key={`exp-${op.id}`} className="bg-gray-800/30">
                        <td colSpan={12} className="px-6 py-4">
                          <div className="grid grid-cols-3 gap-6 text-sm">
                            <div>
                              <p className="text-xs text-gray-500 uppercase mb-1">Motivo tecnico</p>
                              <p className="text-gray-300">{op.motivo_tecnico || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 uppercase mb-1">Emociones</p>
                              <p className="text-gray-300">{op.emociones || '-'}</p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 uppercase mb-1">Errores</p>
                              <p className="text-gray-300">{op.errores || '-'}</p>
                            </div>
                              ...
                              <div className="col-span-3 flex gap-4 mt-2">
                                {op.imagen_antes && (
                                  <div>
                                    ...
                                    <img src={op.imagen_antes} alt="antes"
                                      className="h-32 rounded-lg border border-gray-700 cursor-pointer hover:opacity-80"
                                      onClick={() => window.open(op.imagen_antes)} />
                                  </div>
                                )}
                                {op.imagen_despues && (
                                  <div>
                                    <p className="text-xs text-gray-500 mb-1">Despues</p>
                                    <img src={op.imagen_despues} alt="despues"
                                      className="h-32 rounded-lg border border-gray-700 cursor-pointer hover:opacity-80"
                                      onClick={() => window.open(op.imagen_despues)} />
                                  </div>
                                )}
                              </div>
                            )}
